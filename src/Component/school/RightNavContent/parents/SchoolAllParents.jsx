...
import { schoolParentRequest, schoolParentReset } from '@/redux/slices/school/schoolParent';
import { Button } from '@mui/material';
import { Input, Select } from 'antd';
import { useFormik } from 'formik';
import { RxCross1 } from 'react-icons/rx'
import { arrayObjectFlat, convertToCSV } from '@/utils/constant';
import ParentsList from '@/Component/admin/Table/ParentsList';

const SchoolAllParents = ({ Class }) => {

    const schoolParentState = useSelector(state => state.schoolParent)
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            if (values.id)
                dispatch(schoolParentRequest(`pk=${values.id}`))
            else {
                let payload;
                ...
                dispatch(schoolParentRequest(payload))
            }
        },
    });
    ...
    useEffect(() => {
        if (Class) {
            ...
            dispatch(schoolParentRequest(`page_size=${pageSize}`))

    }, [Class, pageSize])

    useEffect(() => {
        if (schoolParentState.isSuccess) {
            setResponse(schoolParentState.data?.data)
            setData(schoolParentState.data?.data?.results)
            dispatch(schoolParentReset())
        }
    }, [schoolParentState.isSuccess])

    const handlePagination = (requestUrl) => {
        dispatch(schoolParentRequest(`${requestUrl.split('?')[1]}`))
    }
...
export default SchoolAllParents
